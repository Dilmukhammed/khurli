...
const CulturalProverbsModule = () => {
    ...
    const formatAnswersForAI = (answerObject) => { // This function is used by getTaskDetailsForAI
    ...
    const getTaskDetailsForAI = (taskKey) => {
        ...
        if (taskKey === 'beginnerTask1') {
            ...
            if (Object.keys(taskCorrectAnswers).length === 0 && interaction_type_to_send === 'explain_mistakes') {
                // console.warn(`Task ${taskKey} is 'explain_mistakes' but has no correct answers defined in getTaskDetailsForAI.`); // Commented out dev warning
            }
        }
    ...
    const handleAskAI = async (taskKey, userQuery = '') => {
        // console.log('[CulturalProverbsModule] handleAskAI called. taskKey:', taskKey, 'userQuery:', userQuery);
        ...
        if (!block_context) {
            console.error("[CulturalProverbsModule] Could not get task details for AI. taskKey:", taskKey); // Keep this error
            setChatMessages(prev => [
        ...
        // console.log('[CulturalProverbsModule] Calling service with userQuery:', userQuery);
