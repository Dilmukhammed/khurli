...
const translations = {
    ru: {
        pageTitle: "Факт или Мнение", mainTitle: "Факт или Мнение",
        beginnerLevel: "Начальный уровень – Определение и категоризация фактов и мнений",
        factBTask1Title: "1. Сортировка: Факт или Мнение", factBTask1Desc: "Прочитайте каждое предложение и решите, является ли оно фактом (✔️) или мнением (❌).",
        fact: "Факт", opinion: "Мнение", checkAnswersBtn: "Проверить ответы", submitBtn: "Отправить",
        askAiBtn: "Спросить ИИ", closeAiChat: "Закрыть чат ИИ", aiThinking: "ИИ думает...",
        askAiInstruction: "Что бы вы хотели спросить по этому заданию?", aiError: "Не удалось получить ответ от ИИ.",
        allCorrectMessage: "Все ответы верны!", someIncorrectMessage: "Некоторые ответы неверны. Проверьте выделенные.",
        factBTask2Title: "2. Преобразование мнений в факты и наоборот", factBTask2Desc: "Перепишите следующие мнения как факты и наоборот. Примеры даны в учебном материале.",
        rewriteAsOpinion: "Перепишите как мнение:", rewriteAsFact: "Перепишите как факт:",
        placeholderRewriteOpinion: "Ваше мнение...", placeholderRewriteFact: "Ваш факт...",
        factBTask3Title: "3. Игра \"Это факт?\"", factBTask3Desc: "Прочитайте каждое утверждение. Определите, факт это или мнение.",
        intermediateLevel: "Средний уровень – Обоснование фактов и мнений",
        factITask1Title: "1. \"Почему это факт или мнение?\"", factITask1Desc: "Прочитайте каждое утверждение и объясните, почему это факт или мнение.",
        placeholderExplanation: "Объяснение...",
        factITask2Title: "2. Дебаты: Факт или Мнение", factITask2Desc: "Вам дано утверждение. Приведите аргументы, является ли оно фактом или мнением.",
    ...
    en: {
        pageTitle: "Fact or Opinion", mainTitle: "Fact or Opinion",
        beginnerLevel: "Beginner Level – Identifying and Categorizing Facts & Opinions",
        factBTask1Title: "1. Fact or Opinion Sorting", factBTask1Desc: "Read each sentence and decide whether it is a fact (✔️) or an opinion (❌).",
        fact: "Fact", opinion: "Opinion", checkAnswersBtn: "Check Answers", submitBtn: "Submit",
        askAiBtn: "Ask AI", closeAiChat: "Close AI Chat", aiThinking: "AI Thinking...",
        askAiInstruction: "What would you like to ask about this task?", aiError: "Failed to get AI response.",
        allCorrectMessage: "All answers are correct!", someIncorrectMessage: "Some answers are incorrect. Check the highlighted ones.",
        factBTask2Title: "2. Changing Opinions into Facts & Vice Versa", factBTask2Desc: "Rewrite the following opinions as facts and vice versa. Examples are provided in the learning material.",
        rewriteAsOpinion: "Rewrite as an opinion:", rewriteAsFact: "Rewrite as a fact:",
        placeholderRewriteOpinion: "Your opinion...", placeholderRewriteFact: "Your fact...",
        factBTask3Title: "3. \"Is It a Fact?\" Game", factBTask3Desc: "Read each statement. Determine if it is a fact or an opinion.",
        intermediateLevel: "Intermediate Level – Justifying Facts and Opinions",
        factITask1Title: "1. \"Why is it a Fact or Opinion?\"", factITask1Desc: "Read each statement and explain why it is a fact or opinion.",
        placeholderExplanation: "Explanation...",
        factITask2Title: "2. Fact or Opinion Debate", factITask2Desc: "You are given a statement. Argue whether it is a fact or an opinion.",
...
const beginnerTask1Data = [ /* ... existing data ... */ ];
const beginnerTask3Data = [ /* ... existing data ... */ ];
const advancedTask1Data = [ /* ... existing data ... */ ];
// Restore full data if it was shortened in previous logs
const bTask2Items = [
...
const MultipleChoiceTask = ({ taskKey, questions, lang, title, description, options, checkBtnText, allCorrectMsg, someIncorrectMsg, isCompleted = false, onAnswersChecked, onAskAi, showAskAiButton, isMainAiLoading, activeAiTaskKey }) => {
    ...
    const handleCheckAnswersInternal = () => {
        ...
        questions.forEach(q => {
            if (answers[q.id]) { anyAnswered = true; if (answers[q.id] === q.answer) newResults[q.id] = 'correct'; else { newResults[q.id] = 'incorrect'; allCorrect = false; } } else allCorrect = false;
        });
        setResults(newResults);
        if (!anyAnswered) setFeedback(''); else if (allCorrect) setFeedback(allCorrectMsg); else setFeedback(someIncorrectMsg);
...
export default function FactOpinionModule() {
    ...
    // useEffect to load text answers from localStorage on mount
    useEffect(() => {
        if (typeof window !== 'undefined' && window.localStorage && user && user.id) {
            const tasksToLoad = [
                { taskKey: 'bTask2', items: bTask2Items.map(i => i.itemKey) },
                { taskKey: 'iTask1', items: iTask1Items.map(i => i.itemKey) },
                { taskKey: 'iTask2', items: iTask2Items.map(i => i.itemKey) },
                { taskKey: 'aTask2', items: aTask2Items.map(i => `${i.idPrefix}_exp`) } // For aTask2, only text explanations are saved this way
            ];

            const allLoadedAnswers = {};

            tasksToLoad.forEach(task => {
                const loadedAnswersForTask = {};
                task.items.forEach(itemKey => {
                    const storageKey = `fact-opinion-${task.taskKey}-${itemKey}-${user.id}`;
                    const savedValue = localStorage.getItem(storageKey);
                    if (savedValue !== null) {
                        loadedAnswersForTask[itemKey] = savedValue;
                    }
                });
                if (Object.keys(loadedAnswersForTask).length > 0) {
                    allLoadedAnswers[task.taskKey] = loadedAnswersForTask;
                }
            });

            if (Object.keys(allLoadedAnswers).length > 0) {
                setAnswers(prev => {
                    const newState = {...prev};
                    for (const taskKey in allLoadedAnswers) {
                        newState[taskKey] = {
                            ...(prev[taskKey] || {}),
                            ...allLoadedAnswers[taskKey]
                        };
                    }
                    return newState;
                });
    ...
    }, [user, isAuthenticated]); // Re-run if user changes, or on initial auth status change
    ...
    const handleAnswerChangeFactOpinion = useCallback((taskKey, itemKey, value) => {
        const lockableTasks = ['bTask1', 'bTask3', 'aTask1']; // Tasks that lock after correct completion
        if (isTaskCompleted(taskKey) && lockableTasks.includes(taskKey)) {
            return;
        }

        setAnswers(prev => ({ ...prev, [taskKey]: { ...prev[taskKey], [itemKey]: value } }));

        const tasksWithLocalStorage = ['bTask2', 'iTask1', 'iTask2', 'aTask2'];
        if (tasksWithLocalStorage.includes(taskKey) && typeof window !== 'undefined' && window.localStorage && user && user.id) {
            const storageKey = `fact-opinion-${taskKey}-${itemKey}-${user.id}`;
    ...
    }, [isTaskCompleted, user]);

    const handleSubmitFactOpinion = useCallback((taskKey, dataFromChild) => {
        const lockableMCQTasks = ['bTask1', 'bTask3', 'aTask1'];
        if (isTaskCompleted(taskKey) && lockableMCQTasks.includes(taskKey)) {
             return;
        }
        // For non-lockable text tasks (bTask2, iTask1, aTask2) and discussion (iTask2),
        // allow "submission" to proceed for AI interaction and localStorage save, even if marked for progress.

        setShowAiButtons(prev => ({ ...prev, [taskKey]: true }));
        setCurrentErrors(prev => ({ ...prev, [taskKey]: null }));

        let shouldMarkCompleteForProgress = false;
        let isCorrectForMCQ = false;
        if (dataFromChild && dataFromChild.allCorrect !== undefined) isCorrectForMCQ = dataFromChild.allCorrect;

        if (lockableMCQTasks.includes(taskKey)) {
            if (isCorrectForMCQ) shouldMarkCompleteForProgress = true;
        } else if (['bTask2', 'iTask1', 'iTask2', 'aTask2'].includes(taskKey)) {
            shouldMarkCompleteForProgress = true;
        }

        if (shouldMarkCompleteForProgress && isAuthenticated && !isTaskCompleted(taskKey)) {
            moduleService.markTaskAsCompleted('fact-opinion', taskKey)
                .then(() => setCompletedTasks(prev => ({ ...prev, [taskKey]: true })))
                .catch(err => console.error(`Error saving progress for ${taskKey}:`, err));
        } else if (shouldMarkCompleteForProgress && !isAuthenticated) {
            if (lockableMCQTasks.includes(taskKey) && isCorrectForMCQ) {
        ...
        const tasksToSaveOnSubmit = ['bTask2', 'iTask1', 'iTask2', 'aTask2'];
        if (tasksToSaveOnSubmit.includes(taskKey) && typeof window !== 'undefined' && window.localStorage && user && user.id) {
            const taskAnswers = answers[taskKey] || {};
            Object.entries(taskAnswers).forEach(([itemKey, value]) => {
                const storageKey = `fact-opinion-${taskKey}-${itemKey}-${user.id}`;
                try { localStorage.setItem(storageKey, String(value)); }
                catch (e) { console.error(`Error saving ${taskKey} to localStorage on submit:`, e); }
            });
        }
    }, [isAuthenticated, isTaskCompleted, answers, user]);

    const getTaskDetailsForAI_FactOpinion = useCallback((taskKey, additionalData = {}) => {
        // ... (existing getTaskDetailsForAI_FactOpinion logic - assumed correct from previous steps) ...
        ...
        if (taskKey === 'bTask1' || taskKey === 'bTask3' || taskKey === 'aTask1') {
            ...
            const originalStatements = bTask2Items; // Use defined items
            ...
            originalStatements.forEach(stmt => {
                const originalStatementText = stmt.langText[lang] || stmt.langText.en, rewriteInstruction = t[stmt.rewritePromptKey], userAnswer = taskSpecificAnswers[stmt.itemKey] || 'No rewrite provided.';
                contextParts.push(`Original Statement (${stmt.type || 'N/A'}): "${originalStatementText}"\nInstruction: ${rewriteInstruction}`); // Added stmt.type
            ...
            const originalStatements_iTask1 = iTask1Items; // Use defined items
            ...
            originalStatements_iTask1.forEach(stmt => {
                const originalStatementText = stmt.langText[lang] || stmt.langText.en, userAnswer = taskSpecificAnswers[stmt.itemKey] || 'No explanation.';
            ...
            const originalStatements_iTask2 = iTask2Items; // Use defined items
            ...
            originalStatements_iTask2.forEach(stmt => {
                const originalStatementText = stmt.langText[lang] || stmt.langText.en, userAnswer = taskSpecificAnswers[stmt.itemKey] || 'No arguments.';
            ...
            const originalStatements_aTask2 = aTask2Items; // Use defined items
            ...
            originalStatements_aTask2.forEach(stmt => {
                const originalStatementText = stmt.textKey[lang] || stmt.textKey.en, userChoice = taskSpecificAnswers[`${stmt.idPrefix}_choice`] || 'Not answered', userExplanation = taskSpecificAnswers[`${stmt.idPrefix}_exp`] || 'No explanation.';
    ...
    const handleAskAiFactOpinion = useCallback(async (taskKey, userQuery = '', initialDataPayload = null) => {
        // ... (existing handleAskAiFactOpinion logic - assumed correct from previous steps) ...
        ...
        <div className="bg-gray-100 text-gray-800 font-sans">
            <main className="container mx-auto px-6 py-12">
                ...
                <section className="level-section mb-12 p-6 bg-white rounded-lg shadow">
                    ...
                    <div className="mb-8 p-6 border rounded-lg task-card bg-white shadow-sm hover:shadow-md transition-shadow">
                        <h3 className="text-xl font-semibold mb-3">{t.factBTask2Title} {/* No (Completed) for bTask2 */}</h3>
                        <p className="mb-4 text-sm text-gray-600">{t.factBTask2Desc}</p>
                        <div className="space-y-6">
                            {bTask2Items.map((item, index) => (
                                <div key={item.itemKey}>
                                    ...
                                    <input type="text" className="mt-1 w-full border rounded-md p-2 text-sm border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-gray-100" placeholder={t[item.placeholderKey]} onChange={(e) => handleAnswerChangeFactOpinion('bTask2', item.itemKey, e.target.value)} value={answers.bTask2?.[item.itemKey] || ''}
                                        disabled={progressLoading || (isAiLoading && activeChatTaskKey === 'bTask2')} />
                                </div>
                            ))}
                        </div>
                        <div><button onClick={() => handleSubmitFactOpinion('bTask2')} disabled={progressLoading || (isAiLoading && activeChatTaskKey === 'bTask2')} className="mt-6 bg-indigo-500 hover:bg-indigo-600 text-white px-5 py-2 rounded-md text-sm font-medium transition duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50">
                            {t.submitBtn} {/* Always Submit for bTask2 */}
                            </button></div>
                        {showAiButtons['bTask2'] && !currentErrors['bTask2'] && (<div><button onClick={() => {handleAskAiFactOpinion('bTask2');setChatMessages([])}} disabled={isAiLoading && activeChatTaskKey === 'bTask2'} className="mt-2 bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-md text-sm font-medium transition duration-300 disabled:opacity-50 disabled:cursor-not-allowed">{t.askAiBtn}</button></div>)}
                ...
                <section className="level-section mb-12 p-6 bg-white rounded-lg shadow">
                    ...
                    <div className="mb-8 p-6 border rounded-lg task-card bg-white shadow-sm hover:shadow-md transition-shadow">
                        <h3 className="text-xl font-semibold mb-3">{t.factITask1Title} {/* No (Completed) for iTask1 */}</h3>
                        <p className="mb-4 text-sm text-gray-600">{t.factITask1Desc}</p>
                        <div className="space-y-6">
                            {iTask1Items.map((item, index) => (
                                <div key={item.itemKey}>
                                    <p className="font-medium">{index + 1}. {lang === 'ru' ? item.originalTextRu : item.originalTextKey}</p>
                                    <textarea className="mt-2 w-full border rounded-md p-2 text-sm border-gray-300 focus:ring-green-500 focus:border-green-500 disabled:bg-gray-100" rows="2" placeholder={t.placeholderExplanation} onChange={(e) => handleAnswerChangeFactOpinion('iTask1', item.itemKey, e.target.value)} value={answers.iTask1?.[item.itemKey] || ''}
                                        disabled={progressLoading || (isAiLoading && activeChatTaskKey === 'iTask1')} />
                                </div>
                            ))}
                        </div>
                        <div><button onClick={() => handleSubmitFactOpinion('iTask1')} disabled={progressLoading || (isAiLoading && activeChatTaskKey === 'iTask1')} className="mt-6 bg-green-500 hover:bg-green-600 text-white px-5 py-2 rounded-md text-sm font-medium transition duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50">
                            {t.submitBtn} {/* Always Submit for iTask1 */}
                            </button></div>
                        {showAiButtons['iTask1'] && !currentErrors['iTask1'] && (<div><button onClick={() => {handleAskAiFactOpinion('iTask1'); setChatMessages([])}} disabled={isAiLoading && activeChatTaskKey === 'iTask1'} className="mt-2 bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-md text-sm font-medium transition duration-300 disabled:opacity-50 disabled:cursor-not-allowed">{t.askAiBtn}</button></div>)}
                        {currentErrors['iTask1'] && <p className="text-red-500 mt-2 text-sm">{currentErrors['iTask1']}</p>}
                        {activeChatTaskKey === 'iTask1' && (<div className="mt-4"><AiChatWindow messages={chatMessages} isLoading={isAiLoading} onSendMessage={(message) => handleAskAiFactOpinion('iTask1', message)} /><button onClick={() => { setActiveChatTaskKey(null); setChatMessages([]); setCurrentErrors(prev => ({...prev, iTask1: null})); }} className="mt-2 text-sm text-gray-600 hover:text-gray-800">{t.closeAiChat}</button></div>)}
                    </div>

                    <div className="p-6 border rounded-lg task-card bg-white shadow-sm hover:shadow-md transition-shadow">
                        <h3 className="text-xl font-semibold mb-3">{t.factITask2Title}</h3> {/* No (Completed) for iTask2 */}
                        <p className="mb-4 text-sm text-gray-600">{t.factITask2Desc}</p>
                        <div className="space-y-6">
                            {iTask2Items.map((item, index) => (
                                <div key={item.itemKey}>
                                    <p className="font-medium">{lang === 'ru' ? item.originalTextRu : item.originalTextKey}</p>
                                    <textarea className="mt-2 w-full border rounded-md p-2 text-sm border-gray-300 focus:ring-green-500 focus:border-green-500 disabled:bg-gray-100" rows="3" placeholder={t.placeholderArguments} onChange={(e) => handleAnswerChangeFactOpinion('iTask2', item.itemKey, e.target.value)} value={answers.iTask2?.[item.itemKey] || ''}
                                        disabled={progressLoading || (isAiLoading && activeChatTaskKey === 'iTask2')} />
                                </div>
                            ))}
                        </div>
                        <div><button onClick={() => handleSubmitFactOpinion('iTask2')} disabled={progressLoading || (isAiLoading && activeChatTaskKey === 'iTask2')} className="mt-6 bg-green-500 hover:bg-green-600 text-white px-5 py-2 rounded-md text-sm font-medium transition duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50">
                            {t.submitBtn} {/* Always Submit for iTask2 */}
                            </button></div>
                ...
                <section className="level-section p-6 bg-white rounded-lg shadow">
                    ...
                    <div className="p-6 border rounded-lg task-card bg-white shadow-sm hover:shadow-md transition-shadow">
                        <h3 className="text-xl font-semibold mb-3">{t.factATask2Title} {/* No (Completed) for aTask2 */}</h3>
                        <p className="mb-4 text-sm text-gray-600">{t.factATask2Desc}</p>
                        <div className="space-y-6">
                            {aTask2Items.map((item, index) => (
                                <div key={item.idPrefix}>
                                    <p className="font-medium">{index + 1}. {lang === 'ru' ? item.textRu : item.textKey}</p>
                                    <div className="mt-2">
                                        <label className="mr-4 inline-flex items-center"><input type="radio" name={`aTask2-${item.idPrefix}_choice`} value="true" checked={answers.aTask2?.[`${item.idPrefix}_choice`] === 'true'} onChange={(e) => handleAnswerChangeFactOpinion('aTask2', `${item.idPrefix}_choice`, e.target.value)}
                                            disabled={progressLoading || (isAiLoading && activeChatTaskKey === 'aTask2')} className="mr-2 h-4 w-4 text-red-600 border-gray-300 focus:ring-red-500 disabled:opacity-70 disabled:cursor-not-allowed"/>{t.true}</label>
                                        <label className="inline-flex items-center"><input type="radio" name={`aTask2-${item.idPrefix}_choice`} value="false" checked={answers.aTask2?.[`${item.idPrefix}_choice`] === 'false'} onChange={(e) => handleAnswerChangeFactOpinion('aTask2', `${item.idPrefix}_choice`, e.target.value)}
                                            disabled={progressLoading || (isAiLoading && activeChatTaskKey === 'aTask2')} className="mr-2 h-4 w-4 text-red-600 border-gray-300 focus:ring-red-500 disabled:opacity-70 disabled:cursor-not-allowed"/>{t.false}</label>
                                    </div>
                                    <input type="text" className="mt-2 w-full border rounded-md p-2 text-sm border-gray-300 focus:ring-red-500 focus:border-red-500 disabled:bg-gray-100" placeholder={t.placeholderFactCheck} onChange={(e) => handleAnswerChangeFactOpinion('aTask2', `${item.idPrefix}_exp`, e.target.value)} value={answers.aTask2?.[`${item.idPrefix}_exp`] || ''}
                                        disabled={progressLoading || (isAiLoading && activeChatTaskKey === 'aTask2')} />
                                </div>
                            ))}
                        </div>
                        <div><button onClick={() => {handleSubmitFactOpinion('aTask2'); setChatMessages([])}} disabled={progressLoading || (isAiLoading && activeChatTaskKey === 'aTask2')} className="mt-6 bg-red-500 hover:bg-red-600 text-white px-5 py-2 rounded-md text-sm font-medium transition duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 disabled:opacity-50">
                            {t.submitBtn} {/* Always Submit for aTask2 */}
                            </button></div>
                        {showAiButtons['aTask2'] && !currentErrors['aTask2'] && (<div><button onClick={() => handleAskAiFactOpinion('aTask2')} disabled={isAiLoading && activeChatTaskKey === 'aTask2'} className="mt-2 bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-md text-sm font-medium transition duration-300 disabled:opacity-50 disabled:cursor-not-allowed">{t.askAiBtn}</button></div>)}
                        {currentErrors['aTask2'] && <p className="text-red-500 mt-2 text-sm">{currentErrors['aTask2']}</p>}
                        {activeChatTaskKey === 'aTask2' && (<div className="mt-4"><AiChatWindow messages={chatMessages} isLoading={isAiLoading} onSendMessage={(message) => handleAskAiFactOpinion('aTask2', message)} /><button onClick={() => { setActiveChatTaskKey(null); setChatMessages([]); setCurrentErrors(prev => ({...prev, aTask2: null})); }} className="mt-2 text-sm text-gray-600 hover:text-gray-800">{t.closeAiChat}</button></div>)}
                    </div>
                </section>
            </main>
        </div>
